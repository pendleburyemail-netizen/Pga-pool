...
export function scoreParticipant(participant, golferMap) {
  const scoredPicks = participant.picks.map(pickName => {
    ...
    return {
      name: pickName,
      // MC/WD picks get null score — excluded from best-4
      score: isCutOrWD ? null : golfer.total,
      // Display explicitly set to status string, never a number
      display: isCutOrWD ? golfer.status : (golfer.displayTotal || formatScore(golfer.total)),
