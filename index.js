...
function scoreClass(score, isString) {
  if (isString || score === null || score === undefined) return 'score-mc';
      ...
      <div className="card">
        ...
        <div className="card-body">
          <div className="notice notice-info" style={{ marginBottom: 16 }}>
            Enter up to {MAX_PARTICIPANTS} participants, each picking {PICKS_PER_PARTICIPANT} golfers.
            Best {BEST_N} of {PICKS_PER_PARTICIPANT} picks count. MC/WD = +20 penalty.
            {!hasField && ' ⏳ Loading this week\'s field from ESPN…'}
            {hasField && ` Field has ${fieldNames.length} players.`}
          </div>
                ...
                <div className="participant-card-body">
                  ...
                  {p.picks.map((pick, idx) => (
                    <div className="form-group" key={idx}>
                      <label className="form-label">Pick {idx + 1}{idx < BEST_N ? ' ★' : ''}</label>
                      <select
                        className="form-select"
                        value={pick}
                        onChange={e => updatePick(p.id, idx, e.target.value)}
                        disabled={!hasField}
                      >
                        <option value="">{hasField ? '— select golfer —' : 'Loading…'}</option>
                        {fieldNames.map(g => (
                          <option key={g} value={g} disabled={allPicks.has(g) && pick !== g}>
                            {g}{allPicks.has(g) && pick !== g ? ' ✓' : ''}
                          </option>
                        ))}
                      </select>
                    </div>
    ...
    <div>
      ...
      {golferData.length === 0 && !loading && (
        <div className="notice notice-warn">
          No live scores yet — the tournament may not have started. Set up picks in ⚙️ Setup, scores will appear automatically.
        </div>
      )}

      {ranked.length === 0 ? (
        <div className="notice notice-info">
          No participants set up yet. Go to ⚙️ Setup to add participants and picks.
        </div>
      ) : (
        <div className="card">
                    ...
                    <tr key={p.id}>
                      ...
                      <td className={`total-cell ${p.total < 0 ? 'score-under' : p.total > 0 ? 'score-over' : 'score-even'}`}>
                        ...
                        {p.best4Count > 0 && p.best4Count < BEST_N && (
                          <div style={{ fontSize: '0.68rem', color: '#888', fontWeight: 'normal' }}>
                            ({p.best4Count} scored)
                          </div>
                        )}
                      </td>
          ...
          <div style={{ padding: '10px 20px', fontSize: '0.75rem', color: '#888', borderTop: '1px solid #eee' }}>
            ★ = counts in best {BEST_N} · 🟨 highlighted = counting toward total · MC/WD = +20 strokes
          </div>
        </div>
      )}
    </div>
    ...
    <div>
      <div className="card">
        <div className="card-header">
          📊 {tournament?.name || 'Tournament'} — Live Leaderboard
        </div>
            ...
            <table className="lb-table">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={g.name}>
                    <td style={{ fontWeight: 'bold', color: '#666', fontSize: '0.85rem' }}>
                      {g.position || (i + 1)}
                    </td>
                    ...
                    <td className={`total-cell ${
                      g.status === 'MC' || g.status === 'WD' ? 'score-mc' :
                      g.total < 0 ? 'score-under' : g.total > 0 ? 'score-over' : 'score-even'
                    }`}>
                      {g.displayTotal || '--'}
                    </td>
                    <td>
                      <span className={`badge ${g.status === 'Active' ? 'badge-live' : 'badge-final'}`}>
                        {g.status}
                      </span>
                    </td>
                  </tr>
...
export default function Home() {
  ...
  const [participants, setParticipants] = useState([emptyParticipant(1), emptyParticipant(2)]);
  ...
  // Derive sorted field names from live data for the Setup dropdowns
  ...
  const pageTitle = tournament?.shortName
    ? `${tournament.shortName} Pool`
    : 'PGA Tour Pool';
      ...
      <header className="header">
        <div className="header-inner">
          <div>
            <h1>⛳ {pageTitle}</h1>
            {tournament?.venue && (
              <div style={{ fontSize: '0.72rem', color: '#9dc9a5', marginTop: 2 }}>
                📍 {tournament.venue}
              </div>
            )}
          </div>
